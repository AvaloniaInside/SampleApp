...
internal abstract class LottieSplashToDrm : IDisposable
{
    ...
    public LottieSplashToDrm(DrmOutput drmOutput)
    {
    ...
    private void UpdateFrame()
    {
        ...
        lock (_sync)
        {
            using var splash = new SplashToDrm(_drmOutput);
            Size = splash.Size;
            ...
            splash.Canvas.SetMatrix(SKMatrix.CreateScale(
                (float)(destRect.Width / sourceRect.Width),
                (float)(destRect.Height / sourceRect.Height)));
            splash.Canvas.SetMatrix(SKMatrix.CreateTranslation(
